...
**顶部·当前兑奖门店（2026-07-20）**：通栏 sticky bar，标明这次扫码归到哪家店。**用自家设备核销时没有设备级门店绑定，靠这里选一次**——单门店账号自动是那家、只读不打扰；多门店账号出下拉选择器（默认主门店，`curStore` state），右侧提示「兑奖会记到这家店；换到别的店请先切换」。**软默认、不卡扫码**（统计-only，不为归因牺牲收银速度）。选定门店 → 核销事件带 store_id；未选（多门店）落「未指定门店」桶（见 §4.8）。

**两栏布局（2026-07-20 重排，修正视觉权重倒挂）**：网格 `minmax(0,1fr) minmax(0,360px)`——**左栏主、右栏 360 次**（原来相反）。对标 Square/Toast 收银屏铁律：一屏一主动作、主动作最大最高对比。
- **左栏·扫码核销（唯一主动作，占主导、垂直居中）**：
  - 大按钮「扫码核销」（绿、最高对比）→ 进入摄像头取景态 → 识别成功自动核销。
  - 「或 输入奖品码」兜底（手动输码 + **描边小按钮**，弱于扫码，守单一焦点）。
  - 核销成功：绿色提示「核销成功 —— 已计入真实到店」。
- **右栏·概览两数**：今日核销 / 累计核销（demo：9 / 86）。**待核销已移除**（属分析、去数据页）。
- **右栏·最近核销**：最近核销流水（最多 4 条，只显示真实数据）。**无核销记录时整块面板不渲染**。
- **右栏·查看完整数据入口**：一行链接「各门店与趋势 · 查看完整数据 →」跳 Reports。**原「奖品券核销」进度面板已移除**（券进度/各门店等分析归数据页，守操作页/分析页分离）。
...
- **活动**：真实到店核销视角（下述）。
- **游戏（2026-07-14 重构：概览→单游戏下钻 + 已下线历史留存）**：参与度视角，**跟"游戏对象"走**（不跟活动走）。**无到店/核销/门店/券**（游戏没有这些）。数据源 `GAME_METRICS`：`{ plays, players, avgPlaySec, dailyAvg, spark, games:[] }`，`games[]` 每项 `{ id,kind,color,c,name,status(live|offline),liveMeta,plays,players,avgPlaySec,dailyAvg?,trend[],ended?,small? }`。
  - ⭐ **玩次含"活动内游玩"**：某游戏被玩的总次数 = 独立上线被玩 + 在绑定它的活动里被玩（同一游戏对象累计）。与 Activities tab 漏斗顶端「扫码玩」**有意重叠**（两 tab 回答不同问题：参与度 vs 转化），文案已注明「包含在活动里玩的 / including plays inside activities」。**推翻**原"独立上线纯玩数据/无奖品"口径与「纯玩，不送奖品」文案。
  - ⭐ **已下线游戏历史留存**：游戏状态机仍只有 draft/live（**不引入第三态**）；数据层用 `status:"offline"` 表示"曾上线过、现已下线"（=展示态，非游戏状态机态）。下线**不删数据**。
  - **两层结构**：**概览** = 游玩次数 hero(总，日均附注) + 玩家数/平均时长(单行 KPI 卡) + **各游戏横向对比条形图**（含已下线游戏、挂 Live/Offline 标、每根条可点、`玩次/玩家/时长` 三维切换、默认当前维度降序）+ ~~每日游玩~~(已砍：与 hero/条形图同一总数、去重)。点任一条 → **单游戏详情**（hero + 玩家/时长 + 每日趋势）。
  - **单游戏 vs 列表**：`games.length===1` → 跳过概览直接进该游戏详情（省一次点击）；≥2 → 概览，详情页顶「‹ 所有游戏」返回。
  - **在跑 vs 已下线详情**：在跑=绿 hero + "上线以来" + 日均 + 近7天柱状；**已下线=灰存档 hero**(降饱和) + eyebrow"在线期间" + 黄提示条「已下线，下面是上线期间数据，去我的游戏重新上线」+ 每日柱状**只画在线段 + 结束竖虚线**(不画空柱) + **顶部日期 pill 隐藏**（今天/近7天对下线游戏无意义）。
  - **小样本**（`small:true`）：逐日柱降级 sparkline + 「数据太少，多跑几天就有了」（对齐活动侧稀疏规则）。
  - **空态判据**：`hasEverLive`（曾上线过任何游戏）取代原 `hasLiveGame`——**只要曾上线有数据、即使现在全下线也走概览/详情**，仅"从没任何游戏上线过"才空态「去我的游戏」。
  - **字体（2026-07-14 设计评审）**：指标数字走**数据字重**非营销字重——hero 54px/800、KPI 36px/700(单位如 `s` 缩小变灰 `.rh-unit`)、全站数字 `tabular-nums` 等宽；Plus Jakarta Sans 的 900(Black) 放大发闷，全站数字统一 700–800。
  - 调试参数：`&tab=game`、`&g=<游戏id: wheel|scratch|fruit|stack>` 直达单游戏详情。〔2026-07-13：完成率→玩的时长；删冗余副标题。IA 分层见 `Desktop/Mozat/kix/[决策] 2026-07-13-KiX数据页IA与指标-三体.md`。〕
...
**日期范围（右上，2026-07-13 三体）**：**上线以来(默认) / 今天 / 近 7 天 / 近 30 天**。活动看板以"活动上线"为锚点累计（Mailchimp/Klaviyo/HubSpot/Voucherify 惯例）→ 默认「上线以来」、**不设 All-time/全部**（活动级"上线以来"即该活动 lifetime；跨活动的账户级 lifetime 属未来的账户总览页，不在此）。
...
4. **每日到店趋势**：柱图（`nonZeroDays ≥ 7` 时）；稀疏（≤6 非零天）→ mini sparkline + 提示，见下方「稀疏/小样本规则」。
5. **〔条件〕各活动带客排名**：仅 **≥2 个 live 活动**时显示（单活动是一根废条）。
6. **〔条件〕各门店到店**：仅 **≥2 门店**时显示；**小样本时隐藏比例条、只列「门店名 + N 人到店」**（见下方规则）。**⭐「未指定门店」桶（2026-07-20）**：`M.byOutlet.unknown > 0` 时在各门店下方追加一根灰色条「未指定门店 · N 人到店」+ 说明「用自家设备核销、没先选当前门店的，仍计入总数」。**归因轴 = 门店是否已归因，不是设备是不是我们的**（`OutletPanel` 收 `unknown` prop；`byOutlet` 自洽求和含 unknown = walkins）。命名用「未指定门店」不用「未知来源」。⚠️ 完全不走核销页的 off-KiX 核销 KiX 无记录、此桶也覆盖不到（要覆盖须强制"在 KiX 标记券已用"，产品决策，未做）。
...
**数据来源**：原型全站统一从 `data.jsx` 的 `DEMO_METRICS`（稀疏演示 `SPARSE_METRICS`）取数（口径自洽）；研发应同样让所有指标来自单一数据源/接口，避免各页口径打架。
...
### 11.1 登录入口：独立 `/login` 页 + 按 IP 分区的统一验证码 + SSO（不是弹窗、不分登录/注册）（2026-07-14 v3，见 §13 + CHANGELOG #93）
- **独立页不用弹窗**：验证码常切到短信/邮箱 App，弹窗在 SPA 一刷新/切回就丢状态；独立页能刷新、能 deep-link、密码管理器更稳（对标 Stripe/Slack/Notion/Linktree 全用独立页）。
- **不分"登录/注册"**：验证码下登录=注册同一动作——验证通过**有账号就进、没有就后端静默建号**（对标 WhatsApp/Grab/Gojek + Notion/Vercel「continue=create account」）。原型 `AuthEntry` 组件：id 步 → otp 步 → 进后台。
- **按 IP 分区展示**（`AuthEntry`，原型用 `?region=cn` 或卡底 demo 链接模拟）：**海外 IP** = 邮箱 OTP + Google/Apple SSO；**国内 IP** = 手机号 +86（Google/Apple 国内不可用）。**SSO 保留**，分裂靠**后端身份归并**（email/kix_user_id 归到同一账号）防——⚠️ 归并覆盖自动建号场景是后端硬验收项。
- **落地页砍掉单独 "Sign up"**：只留 **"免费开始"（建游戏，免登录）** + **"登录"（回访 → `/login`）**。注册仍在发布闸门验证步完成。**发布闸门无「已有账号登录」冗余链**（验证已合一）。
- **发布闸门 与 /login 共用同一 `AuthEntry`，按 canonical 主键去重**：回访商家"免费开始→发布"时若身份已存在 → 不重复建号、直接挂到原账号、跳过补资料。
- **verify 后按 membership 分流**：0→匿名进 / 1→直接进 / ≥2→账号选择页（`AccountPicker`，`?screen=choose`；`?accounts=multi` 模拟）。详见 §13。
- 端到端：① 回访登录→/login→(按IP)邮箱或手机→验证码→(membership 分流)进后台；② 新商家"免费开始"→建游戏→发布闸门→验证(新)→建号→绑卡→恭喜+可跳过补资料→进后台；③ 点登录但无账号→验证码通过→静默建号（文案中性"验证码已发送"，不写"无账号"）。
- 移动端 OTP：`autocomplete="one-time-code"` 短信码自动填充（邮箱码需手动复制，海外走此）；OTP 屏必须是可刷新、保留身份的真实 URL（独立页天然满足）。
- 接口：`POST /api/auth/start{login_id, region}` → 发码；`POST /api/auth/verify{login_id, code}` → `{access_token, refresh_token, brand_id, is_new_account, card_exempt}`（对齐前端方案 §3.1；SSO 走 `third-party/login`）。
