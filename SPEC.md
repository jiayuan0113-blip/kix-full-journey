...
- **活动二维码**：活动编辑器底部，逐门店展示 QR 卡片。上线后：系统自动生成 QR + 「下载」按钮；未上线：灰色占位 + 「上线后自动生成」说明。**两种状态均支持「上传自定义图片」**：商家可上传自行设计的 QR 图（如已印好的台卡），上传后替换系统 QR，二次操作显示「更换」。真实工程需 `PUT /activities/:id/outlets/:oid/qr-image` 接口存储。**主页 Hero 不放 QR**（多门店场景下无法用单一二维码覆盖，正确出口在活动编辑器底部）。
...
- 首屏 Hero：**无眉标** + 标题「把路过的人，变成回头客」+ 副标题「AI做好你的品牌游戏；客人玩游戏赢券进店，**下次自动发券召回。**」+ 双 CTA（**主**：免费创建第一个游戏 → 进创建流程；**次**：看游戏样片 → 页内平滑滚动至 `#gallery-sec` 游戏展示区）+ 真实案例数据条（某冰激凌店 · **48% 到店率 / 1,500 新客到店 / 29% 老客召回复购**）+ 右侧**视频自动播放占位框**（16:9，上线替换为产品演示视频）。次 CTA 不重复主 CTA 动作，让视频自播 + 样片试玩共同完成"说服未决定的访客"职责。
...
- 无 live 活动时：Hero 显示空态「暂无上线活动 / 建个活动就能开门营业」+ 「+ 新建活动」按钮。
