...
**开业跑道模型（2026-07-22 重构，⭐推翻旧"单 CTA hero + 竖直上手清单"）**：主页非营业态串成一条「开业跑道」——hero 是**唯一带动作**的元素，下方一条**横向进度条 `HomeRunway`（纯状态、不可点）**：做好游戏 ✓ → 上线活动 → 贴门店码 → 到店兑奖。分水岭 `hasWalkins`（到店兑奖≥1）→ 营业态；否则按 (`liveAct` / `qrDownloaded` / `liveGame`) 落 C/B/A1/A2。

**核心原则**：
- **home 常驻主动作 = 扫码兑奖（收银台）**，不是数据展示、也不是一次性的下载。数据只当二级一瞥（营业态成本条 + 数据页）。
- **下载门店二维码 = 一次性 bootstrap**（仅 A1 当主按钮）；**一旦下载完成，home 永久锚定"扫码兑奖"**（A2 → 营业态）。分水岭 `qrDownloaded`（A1↔A2）+ `hasWalkins`（A2↔营业态）。
- **免费试用信息「首 3 个月免费 · 随时取消」只在上线前出现（C）**；已上线态（B/A1/A2）不显（避损失厌恶 + 已承诺无需再劝）。
- **无 QR 码贴 Hero**：活动 QR 逐门店，正确出口在活动编辑器底部；A1「下载门店二维码」跳该出口/触发下载。

**五态**：
| 态 | 判据 | Hero（深色块） | 跑道 |
|---|---|---|---|
| **C 全空** | 无 live 活动 + 无 live 游戏 | 灰徽章「游戏做好了 · 游戏名」+ 标题「上线活动，把客人请到店」+ 主按钮「上线活动」+ 理由 micro「送券引客，到店兑奖」；下方**引导条**（游戏图标 +「还没想好？先让客人玩玩看 / 只上线游戏、不送券，随时能再加活动」+ ghost「只上线游戏」）；免费试用 micro | 当前=上线活动 |
| **B 游戏 live** | 有 live 游戏、无 live 活动 | 绿「进行中」+ 游戏名 + 今天玩了 + 主按钮「+ 上线活动」+ 理由 micro「上线活动，把客人请到店」。**不谎报"还没有活动"** | 当前=上线活动 |
| **A1 活动刚上线** | live 活动、门店码未下载 | 进行中 + 活动名 + 主按钮「下载门店二维码」(一次性) + micro「每家店一张，贴在客人扫得到处」 | 当前=贴门店码 |
| **A2 收银台待客** | live 活动、门店码已下载、到店兑奖=0 | 进行中 + 活动名 + 今天玩了/到店兑奖 + 主按钮「扫码兑奖」。**无跑道**（onboarding 毕业） | — |
| **营业态** | 到店兑奖≥1 | LIVE + 活动名（可点）+ 今日三数（plays/到店/券剩余）+ 全宽「扫码兑奖」+ 多门店分条(`outlet-bar`, 门店≥2) + 成本条 + 最近 + 召回 | — |

- **判据**：`liveAct = activities.find(status==='live')`；`liveGame = myGames.find(status==='live')`；`qrDownloaded`（下载门店码后置 true，一次性）；`hasWalkins`（到店兑奖≥1）。工程侧同源。
- **`HomeRunway` 组件**：`cur` = 当前步 index（C/B=1 上线活动，A1=2 贴门店码）；done 打勾、cur 绿环高亮、future 灰；只在 C/B/A1 渲染（A2/营业态不显）。
- ⚠️ **跑道第 1 节「做好游戏」被动打勾、不可点**：上线游戏非上线活动前提，且上线活动会自动上线其依附游戏（§3.9），做成可点会造假顺序 + 永远打不上的勾。
...
##### 首次登录 / 无 live 活动的主页（开业跑道，五态详见 §4.5）
