...
已登录（App Shell，常驻左侧栏 6 项）
  🏠 主页 Home   ← 登录后默认落点；底部「+ 新建活动」；无活动时顶部引导卡
...
- **每张券字段（5 列卡片）**：`name 奖品名(如卡布奇诺)` · `price 原价` · `discount 折扣(免费/买一送一/8折)` · `qty 张数` · `perCust 每人限领`；运行期加 `awarded 已发` / `redeemed 已核销` / `remaining=qty−awarded`。
...
- **活动编辑器**（`ActivityEditor`）：从上到下 = 活动名称 → 参与门店（`OutletScope`） → 奖品券（`VoucherEditor`） → 选游戏（展示用户已创建的所有游戏大卡片，点选即绑定；卡片下方「选择」「查看详情」按钮；末尾「+ 新建游戏」卡） → 底部上线/下线 + 保存。
...
### 4.1 落地页 Landing
- 顶栏：logo（真图 `logo.png`）/ 玩法·怎么用·价格 / EN中文 / 登录 / 免费开始。
- 首屏：**无角标**（已删「给街边小店…」）+ 标题「把路过的人，变成回头客」+ 副标题（客人玩游戏赢券，凭券进店。**老客太久没来，自动召回。**）+ 双 CTA（免费创建第一个游戏 / 看怎么玩）+ 真实战绩三数（48% 到店率 · 1,500 新客到店 · 29% 老客召回复购）+ 右侧**可玩转盘**（手机内，自动转一次）。
- 闭环带：扫码玩 → 赢券 → 到店核销（二维码/滑动）→ 老客自动召回。
- 其余区：一套搞定三件事（01 拉新客 / 02 召回老客 / 03 零浪费——没进店不花一分钱，S$0 曝光费 vs 传统广告 90% 预算浪费）/ 游戏样片（8 个，1000+ 模板）/ 三步上线 / 价格（免费 S$0 · 专业 S$49 · 连锁 联系我们）/ 结尾绿色 CTA / 页脚。
- 所有 CTA → 建游戏（未登录）。
- 已删除：首屏 CTA 下「免费开始·只为真实到店付费·无需信用卡」小字。
...
- **登录后＝2 步**（步骤条：选游戏 / 预览发布）：⭐ 点「新建游戏」**跳过描述与 loader，直接到选游戏**（理由：游戏本身不分拉新/复购，是机制决定的，再问目标是噪音）→ 预览发布。
...
1. **活动名称**（panel）：一个输入框，placeholder「例如：周末咖啡促销」。
...
- **右栏·概览三数**：今日核销 / 待核销（已发未核销） / 累计核销。
- **右栏·奖品券核销（简要）**：每券一行 = 名称·折扣 + `已核销/总张数`（如 41/100）+ 单段进度条。顶部「查看完整数据 →」跳 Reports。
...
**设计逻辑**：数据页 = 店主坐下来复盘的地方。看的是全貌——哪个活动效果好、到店趋势、新客回头客比例、各门店贡献、券消耗进度。不做实时操作（那是核销页和主页的事）。

**日期切换**：今天 / 近 7 天 / 近 30 天。

**4 个 KPI（按漏斗顺序）**：玩了游戏 → 到店核销 → 新客到店 → 回头客到店。每个含数字 + 环比箭头 + sparkline。

**图表区**：
- 每天到店趋势柱图。
- 新客 vs 回头客环图。
- 哪个**活动**带客（按到店人数排序，右上角「管理活动 →」跳活动页）。
- 各门店核销（每店核销张数，库存跨门店共享）。
- 奖品券剩余（每券三段条：已核销 / 待核销 / 还可发）。
- 最近到店流水。
