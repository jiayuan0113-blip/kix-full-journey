...
> **2026-07-20 更新（详见 CHANGELOG #98/#99）**：
> - **落地页「进地图」重定位**：hero = 嵌入式消费者城市地图（需求侧可视化），叙事 lead-with-demand（卖需求不卖功能）；第二屏 = 多手机屏客人流程 **城市地图 → 你的 3D 店 → 玩游戏 → 赢券 → 到店兑**（前两屏用 3D 渲染图 `walkthrough/map-3d.png`、`store-3d.png`，后三屏 `play/win/redeem.png`）；网络段「一整座城/全世界的游戏乐园」；SeeYourGame = **live-3D 店铺预览**（模拟，研发接真实 3D embed：three.js / model-viewer）；Stories/店主证言 + 「每家店都值得拥有自己的游乐场」。
> - **产品事实**：每家店 = 一个可进入的 **3D 游戏化建模**，内玩该店专属小游戏；客人三层下钻 城市地图→店 3D 建模→店内游戏。
> - **建游戏 3 步 UI**：描述(店型线性图标 chips) → 选游戏(店铺卡 + 匹配% + 方形封面 + 2 候选) → 我的游戏(配色/Logo/商品图/AI 抓取/🚀上线)；**删除 building loader 屏**（描述→选游戏直连）。方形游戏封面、live-3D、真实门店 3D 建模均为研发待接的真实资产。
