...
> **2026-07-23 更新（落地页两幕重构；详见 CHANGELOG #107 + `design-runs/落地页两幕重构/00-方向.md`。取代下方 07-21 note 的落地页顺序/地图/第三屏部分）**：
> - **落地页顺序**（`Landing()`）：Hero → Walkthrough(5 手机屏) → **地图节(`WhyGame`, id `map`)** → **为什么有效(`FairDeal`, id `why`)** → Pricing → FAQ → 终屏 CTA。叙事 = 工具(游戏)→网络(地图)→为什么有效→价格→行动；不加显式"第一幕/第二幕"章节壳。
> - **删** `SeeYourGame`（07-21 note 的"第三屏"作废；函数保留未渲染）；**删** hero `.hero-chips`（3分钟上线/免费试用）。
> - **地图节改版式**：整幅地图图（`<img src="landing/map.png">`，占位待换正式图）+ **4 步横排在下**（`.mapsteps`；旧 2 列 map/steps 作废）。`MapHero` mock 已不在落地页渲染。
> - **「为什么有效」= 3 卡（三体确定，覆盖店主决策三问）**：① 拉新「让路过的人玩着就进店」② 留存「太久没来的客人，请他们回来」= **用比赛/品牌活动/新游戏召回**（⚠️ 修正：非"自动发券"）③ 成本「每一块钱都对着一个真客人」。
> - **文案诚实化**：撤回「不打折/不烧广告费」绝对说法（发券=让利、送奖品=获客成本，多疑店主一戳就破）→ 全线「不买广告位 / 钱跟着真客人走 / 券有上限只在到店兑现时让利 / 只按真玩的人计 MAU」。
> - ⚠️ **canon 待办**：pricing「老客自动召回/Auto win-back」+ 未渲染 `ThreeThings` 卡2「30天自动发券」与 ②新口径冲突，待统一；地图标题「让全球人发现」薄网络下略满，建议改「让附近玩家发现你」，待定。
>
> **2026-07-21 更新（落地页再定位；详见 CHANGELOG #101，取代 #98 的地图版）**：
> - **落地页 = 视频版 hero + 游戏优先叙事，地图降为辅助段**（#98「hero 进地图」已被取代；地图完整版存 `origin/landing-map` 分支备查）：
>   - **Hero** = **16:10 长方形产品视频占位**（播放键 + 实拍演示徽标 + 活动浮层；研发接真实产品视频）；副题游戏优先。
>   - **第二屏 Walkthrough** = 旧游戏优先 5 步 **海报 → 扫码免下载 → 玩专属游戏 → 赢券 → 到店兑奖**（`poster/play/win/redeem.png`），标题「把你的店变成游乐场」；**无城市地图/3D 店开场**。
>   - **第三屏 SeeYourGame** = 旧手机游戏截图（`play.png`），**非** live-3D 店。
>   - **地图辅助段（`#why`）** = 标题「把你的店铺小游戏放上地图，让全球人发现」+ 一句小字 + 中号地图卡（`MapHero`）+ **竖向 4 步发现流程**（逛地图→刷到你→玩赢券→到店）。地图定位为 come-for-the-tool 之后的 stay-for-the-network 辅助特征，不是 hero 焦点。
>   - **删** Stories 店主证言段。
>   - `walkthrough/map-3d.png`、`store-3d.png`、`.live3d`、hero 内 `MapHero`/城市地图叙事**在 main 落地页已移除**（仅 `origin/landing-map` 分支保留）。
> - **产品事实（不变）**：每家店 = 一个可进入的 **3D 游戏化建模**，内玩该店专属小游戏；客人三层下钻 城市地图→店 3D 建模→店内游戏。（落地页当前弱化呈现，非否定该事实。）
> - **建游戏 3 步 UI（#99，不变）**：描述(店型线性图标 chips) → 选游戏(店铺卡 + 匹配% + 方形封面 + 2 候选) → 我的游戏(配色/Logo/商品图/AI 抓取/🚀上线)；**删除 building loader 屏**。方形游戏封面为研发待接的真实资产。
...
**两阶段模型（2026-07-22 重构，⭐推翻旧"单 CTA hero + 竖直上手清单"；支持多活动/多游戏聚合）**：主页只有两个阶段——**① 开业跑道**（上线第一个活动的单活动旅程，hero 唯一动作 + 横向进度条 `HomeRunway`）→ **② 运营台**（在跑，业务级聚合，任意数量活动/游戏，无进度条）。**你不会同时 onboard 两个活动**；一旦有多个活动/有到店，就进运营台。
- `HomeRunway`（纯状态、不可点）：做好游戏 ✓ → 上线活动 → 贴门店码 → 到店兑奖。
- **数据不再 `find` 第一个 live**：`liveActs = activities.filter(live)`、`liveGames = myGames.filter(live)`；`operating = nAct≥1 && (qrDownloaded || hasWalkins || nAct≥2)`。

**核心原则**：
- **home 常驻主动作 = 扫码兑奖（收银台）**，不是数据展示、也不是一次性的下载。数据只当二级一瞥（成本/游玩一瞥条 → 数据页）。
- **下载门店二维码 = 一次性 bootstrap**（仅 A1 当主按钮）；下载完成后 home 永久锚定"扫码兑奖"。
- **免费试用信息「首 3 个月免费 · 随时取消」只在上线前出现（C）**；已上线态不显（避损失厌恶）。运营台成本条的试用状态是账单仪表、非劝说 micro，保留。
- **扫码兑奖与是哪个活动无关**（收银员扫任意中奖码）→ 多活动下主动作不变。**逐活动明细在「活动」页和「数据」页**，home 只做业务级一瞥（守单一焦点）。

**① 开业跑道三态**（单活动 + 进度条）：
| 态 | 判据 | Hero（深色块） | 跑道 |
|---|---|---|---|
| **C 全空** | 无 live 活动 + 无 live 游戏 | 灰徽章「游戏做好了 · 游戏名」+ 标题「上线活动，把客人请到店」+ 主按钮「上线活动」+ 理由 micro「送券引客，到店兑奖」；下方**圆角引导条**（游戏图标 +「还没想好？先让客人玩玩看 / 只上线游戏、不送券，随时能再加活动」+ ghost「只上线游戏」）；免费试用 micro | cur=上线活动 |
| **B 游戏 live·无活动** | 无 live 活动、有 live 游戏 | 绿「进行中」+ **游戏名（1 个）/「N 个游戏在跑」（≥2，点→我的游戏）** + 今天玩了 + 主按钮「+ 上线活动」+ 理由 micro「上线活动，把客人请到店」 | cur=上线活动 |
| **A1 活动刚上线** | 恰 1 个 live 活动、门店码未下载、无到店 | 进行中 + 活动名 + 主按钮「下载门店二维码」(一次性) + micro「每家店一张，贴在客人扫得到处」 | cur=贴门店码 |

**② 运营台**（业务级聚合，无进度条；触发 = `qrDownloaded` 或 `hasWalkins` 或 `nAct≥2`）：
- **Hero**：绿「进行中」+ 标题 **1 活动=活动名（点→该活动编辑器）/ ≥2 活动=「N 个活动进行中」（点→活动页并预选「已上线」筛选）** + 今日 **玩了 / 到店兑奖**（跨活动聚合）+ **券剩余（仅 1 活动时显；多活动歧义→交券提醒条）** + 全宽「扫码兑奖」+ 多门店分条(`outlet-bar`, 门店≥2)。
- **二级一瞥条 → 数据页（历史看板）**：`hasWalkins` → 「本月新客 N + 试用状态」；未到店 → 「N 上线以来玩过 · 看完整数据和趋势」。
- **券提醒**（lowStock）：跨所有 live 活动扫描、显券剩余最紧张的一个（发完就停=静默失败）。
- **最近**：`hasWalkins` → 兑奖/新客流水（FEED）；未到店 → **游玩动态**（有人玩了游戏 / 新玩家扫码进来 / 连玩 3 局，此阶段无兑奖流水、守"有真实数据才渲染"）。
- **召回卡**（仅 `hasWalkins`）：「18 位老顾客 30 天没来」→ 一键召回 → 就地成功态。

- **`HomeRunway` 组件**：`cur` = 当前步 index（C/B=1 上线活动，A1=2 贴门店码）；只在开业跑道三态渲染。
- ⚠️ **跑道第 1 节「做好游戏」被动打勾、不可点**（上线游戏非上线活动前提，做成可点会造假顺序）。
- **导航**：`ActivitiesView` 收 `initFilt`；`AppShell` `actFilt` state + `goActs(f)`；聚合标题走 `onGoActivitiesLive=goActs("live")`。
...
##### 首次登录 / 无 live 活动的主页（开业跑道，两阶段模型详见 §4.5）
- **判据**：无 `status==='live'` 的活动（细分：C 无 live 游戏 / B 有 live 游戏 / A1 活动 live 待下载码 / A2 已下载待到店）。
- **只显示**：对应态 hero + `HomeRunway`（A2 无跑道）+（仅 C）免费试用 micro + 引导条。
- **隐藏所有没数据来源的模块**：最近动态流、召回老客卡、侧栏「核销」角标。统计/动态/召回**必须有真实数据才渲染**，绝不显示编造数字或 0。
- 原型：`publishDone` 全新商家首次发布置空 activities；调试 `?fresh=1`(空活动) + `?nogame=1`(C 无 live 游戏) / `?nowalkins=1`(到店兑奖=0 → A1) / `?qr=1`(门店码已下载 → A2)。
