...
- ⭐ **赢奖门槛 `winScore`（活动级，2026-07-02 新增）**：在「按张数自然发放」之上叠加一道**达标门槛**——玩家须在游戏中达到 `winScore` 才获得赢券资格（未达标不发）。数值单位**视游戏而定**（分数 / 关卡 / 回合），文案通用化不写死「分」。默认 1000。**仍无中奖率概念**——达标是确定性的、由玩家操作决定，不是概率。UI 在活动编辑器「游戏」板块下（`.win-cond`）。
...
- 样片：**「上千种玩法，每个都能变成你的品牌」**（讲多样+能换品牌，不讲流程）+ 8 样片 + 1000+ 模板。8 款命名规则=**「店型-玩法类型」**（咖啡店-配方游戏 / 甜品店-合成游戏 / 奶茶店-接珍珠游戏 / 果茶店-消除游戏 / 美妆店-连连看 / 服装店-翻牌游戏 / 水果店-三消游戏 / 便利店-叠叠消除），小字=「怎么玩」一句话。中英双语。
- 三步：**「三步做好，开门收客」**（流程唯一出处）。
...
5. **活动二维码**（panel）：
   - **逐门店卡片**（一门店一码，用于归因）。**任意状态（草稿/审批/上线）都显示真码 + 「下载」按钮**——它只是指向 Web 玩游戏页的链接，商家可提前下载打印备货。**不接受商家上传自定义图**（入场码必须平台统一生成）。
   - 设计逻辑：二维码是活动级别的（不是游戏级别），因为客人扫码后进入的是这个活动绑定的游戏，赢的券也属于这个活动。审批只 gate 游戏是否可玩，不 gate 二维码是否存在。
