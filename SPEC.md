...
### 4.1 落地页 Landing（更新 2026-07-01）
- 顶栏：logo / **样片**·怎么用·价格 / EN中文 / 登录 / 免费开始。
- 首屏 Hero：**无眉标**（已删，信息合并进副标题）+ 标题「把路过的人，变成回头客」+ 副标题「AI做好你的品牌游戏；客人玩游戏赢券进店，**下次自动发券召回。**」+ 双 CTA（免费创建第一个游戏 / 看 30 秒怎么玩）+ 真实案例数据条（某冰激凌店 · **48% 到店率 / 1,500 新客到店 / 29% 老客召回复购**）+ 右侧**视频占位框**（16:9，上线替换为产品演示视频）。
- 闭环带（**机制的唯一出处**）：扫码玩 → 赢券 → 到店核销（二维码/滑动）→ 老客自动召回。
- 三件事（每卡一个**不重叠**职责）：01 拉新客（只讲"玩着进店"）/ 02 召回老客（30 天没来自动发券，**29% 是它唯一的家**）/ 03 零浪费（**专职 vs 广告**：不像投广告烧钱，S$0 曝光/点击，传统广告 90% 浪费）。
- 样片：**「上千种玩法，每个都能变成你的品牌」**（讲多样+能换品牌，不讲流程）+ 8 样片 + 1000+ 模板。
- 三步：**「三步做好，开门收客」**（流程唯一出处）。
- 价格（**B 的价格主场**）：标题「免费开始，到店才计费」（**无 S$3/位 大字**，S$3 仅在套餐描述里）+ 免费 S$0（按钮「免费开始」）· 专业 S$49（按钮「**立即开始**」）· 连锁（按钮「预约沟通」）。
